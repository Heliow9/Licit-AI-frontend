// src/pages/AnalyzeEdital.jsx
import { useEffect, useRef, useState } from "react";
import { Link } from "react-router-dom";
import { marked } from "marked";
import DOMPurify from "dompurify";

import { apiFetch, buildUrl, getAuthHeaders, getToken, API_BASE } from "../lib/api";
...
function stripInlineSizes(html) {
  return html
    .replace(/\swidth="[^"]*"/gi, "")
    .replace(/\sheight="[^"]*"/gi, "")
    .replace(/\sstyle="[^"]*(?:width|height)\s*:[^"]*"/gi, (m) => {
      return m
        .replace(/width\s*:[^;"]*;?/gi, "")
        .replace(/height\s*:[^;"]*;?/gi, "")
        .replace(/\sstyle="\s*"/, "");
...
function mdToHtml(md) {
  ...
  return normalized.replaceAll("<a ", '<a target="_blank" rel="noopener noreferrer" ');
}
  ...
  useEffect(() => {
    ...
    function startPolling() {
      ...
      pollRef.current = setInterval(async () => {
        try {
          const r = await fetch(buildUrl(`/api/edital/analisar/status/${jobId}`), {
            headers: getAuthHeaders(),
          });
          if (!r.ok) {
            if (r.status === 404) {
...
export default function AnalyzeEdital() {
  ...
  const handleChangeAnexos = (e) => setAnexosFiles(Array.from(e.target.files || []));
  ...
  const handleDropAnexos = (e) => { prevent(e); setAnexosFiles(Array.from(e.dataTransfer.files || [])); };
  ...
  // Responde ao progresso/resultado
  useEffect(() => {
    (async () => {
      if (!jobStatus) return;
      ...
      if (jobStatus.status === "done") {
        try {
          const rr = await fetch(buildUrl(`/api/edital/analisar/result/${jobStatus.id}`), {
            headers: getAuthHeaders(),
          });
          if (!rr.ok) {
            const tx = await rr.text().catch(() => "");
            throw new Error(`HTTP ${rr.status} ${tx}`);
          }
          const res = await rr.json();
          const finalReport = typeof res?.report === "string" ? res.report : "";
          const finalPdf = res?.pdf && res.pdf.url ? res.pdf : null;

          setReport(finalReport);
          setOutputs(finalPdf ? [finalPdf] : []);
          setError("");
        } catch (e) {
          let msg = "Falha ao obter resultado final.";
          const m = e?.message || "";
          try {
            const parsed = JSON.parse(m);
            if (parsed?.error || parsed?.details) {
              msg = [parsed.error, parsed.details].filter(Boolean).join(" — ");
            }
          } catch {
            if (/404/i.test(m) || /Job não encontrado/i.test(m)) {
              msg = "O processamento não foi encontrado (pode ter havido reinício do servidor). Por favor, inicie uma nova análise.";
            } else if (m) {
              msg = `Falha ao obter resultado final — ${m}`;
            }
          }
          setError(msg);
        } finally {
        ...
        setError(jobStatus.phase ? `Falha na análise: ${jobStatus.phase}` : "Falha na análise.");
        localStorage.removeItem("currentJobId");
        setJobId("");
        setLoading(false);
      } else {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch(buildUrl("/api/edital/analisar/start"), {
        method: "POST",
        headers: getAuthHeaders(), // não setar Content-Type manualmente com FormData
        body: form,
      });
      if (!res.ok) {
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-6">
        ...
        <div>
          ...
          <div
            ...
            role="button"
            tabIndex={0}
            onClick={!jobId ? () => editalInputRef.current?.click() : undefined}
            onKeyDown={(e) => (!jobId && (e.key === "Enter" || e.key === " ") ? editalInputRef.current?.click() : null)}
            onDragOver={!jobId ? (e) => { e.preventDefault(); e.stopPropagation(); } : undefined}
            onDrop={!jobId ? (e) => { e.preventDefault(); e.stopPropagation(); const f = Array.from(e.dataTransfer.files||[]).filter(x=>/pdf$/i.test(x.name)); setEditalFiles(mode==="basic"?f.slice(0,1):f);} : undefined}
          >
        ...
        {mode === "super" && (
          <div>
            ...
            <div
              ...
              role="button"
              tabIndex={0}
              onClick={!jobId ? () => anexosInputRef.current?.click() : undefined}
              onKeyDown={(e) => (!jobId && (e.key === "Enter" || e.key === " ") ? anexosInputRef.current?.click() : null)}
              onDragOver={!jobId ? (e) => { e.preventDefault(); e.stopPropagation(); } : undefined}
              onDrop={!jobId ? (e) => { e.preventDefault(); e.stopPropagation(); setAnexosFiles(Array.from(e.dataTransfer.files||[])); } : undefined}
            >
              <div className="text-center">
                <svg
        ...
        <div className="pt-2 flex flex-wrap gap-2">
          ...
          >
            ...
            <span>{loading || jobId ? (mode === "super" ? "Processando..." : "Analisando...") : "Analisar Agora"}</span>
          </button>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        {outputs.length === 0 ? (
          ...
          <ul className="divide-y divide-slate-200">
            {outputs.map((o, idx) => (
              <li key={idx} className="flex flex-col gap-3 py-4 md:flex-row md:items-center md:justify-between wrap-anywhere">
                <div className="min-w-0">
                  <p className="truncate font-semibold text-slate-700">{o.filename || "Relatório de Viabilidade"}</p>
